...
def xolusat_register():
    ...
    if send_email:
        # Obtener contactos Xolusat desde la planilla (con fallback a hardcoded)
        xol_contactos = excel.data.get('xolusat', {})
        to_xol = xol_contactos.get('to', '') or "operaciones@xolusat.com; imartinez@xolusat.com; fmella@xolusat.com; centrodecontrol@xolusat.com"
        cc_xol = xol_contactos.get('cc', '') or "IM.BHD@ncratleos.com; gestion_atm@bhd.com.do; Aurora.Rodriguez@ncratleos.com; Daisy.Perez@ncratleos.com; Alexander.Torres@ncratleos.com; Monica.Serrano@ncratleos.com"
...
@app.route('/api/metricas/<int:idx>', methods=['DELETE'])
def metricas_eliminar(idx):
    if idx < 0 or idx >= len(metricas_sesiones):
        return jsonify({'status': 'error', 'message': 'Índice fuera de rango'}), 404
    metricas_sesiones.pop(idx)
    try:
        with open(METRICAS_FILE, 'w', encoding='utf-8') as f:
            json.dump(metricas_sesiones, f, ensure_ascii=False, indent=2)
    except Exception as e:
