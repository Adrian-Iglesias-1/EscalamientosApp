...
var metDiasMap = {};     // 'YYYY-MM-DD' -> [sesiones]
var metMesesMap = {};    // 'YYYY-MM'    -> [sesiones]
...
// ── XOLUSAT CONTACTOS ──
function renderXolusatContactos() {
    var xol = (contactosData && contactosData.xolusat) || {};
    document.getElementById('contactos-xol-to').value = xol.to || '';
    document.getElementById('contactos-xol-cc').value = xol.cc || '';
}

async function guardarXolusatContactos() {
    var to = document.getElementById('contactos-xol-to').value.trim();
    var cc = document.getElementById('contactos-xol-cc').value.trim();
    if (!to) return showToast('Completá al menos el campo "Para"', 'danger');

    showConfirm('¿Guardar contactos de xolusat?', async function() {
        try {
            var res = await fetch('/api/contactos/guardar', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ custodio: 'XOLUSAT', email: to, cc: cc, aplica_finde: false, tipo: 'xolusat' })
            });
            var data = await res.json();
            if (data.status === 'success') {
                showToast('Contactos XOLUSAT actualizados', 'success');
                loadContactos();
            } else {
                showToast(data.message || data.error || 'Error', 'danger');
            }
...
// Suma un conjunto de sesiones → totales + ritmos (por correo / por ticket)
function aggregar(lista) {
    var correos = 0, envioMs = 0, visionMs = 0, atms = 0;
    lista.forEach(function(s) {
        var d = s.duraciones || {};
        correos  += s.correos_enviados || (s.correos_sucursal || 0) + (s.correos_terceros || 0);
        envioMs  += d.correos_ms || 0;
        visionMs += d.vision_ms || 0;
        atms     += s.n_atms || 0;
    });
    return {
        n: lista.length, correos: correos, envioMs: envioMs, visionMs: visionMs, atms: atms,
        msPorCorreo: correos > 0 ? envioMs / correos : 0,
        msPorTk:     atms > 0    ? visionMs / atms   : 0
    };
}

// HTML de las 3 mini-cards (correos / construcción→envío / vision) para un período
function periodoCardsHtml(a) {
    function card(color, big, label, sub) {
        return '<div class="col-4">' +
            '<div class="card border-0 met-kpi-card h-100" style="border-top-color:' + color + ' !important;">' +
            '<div class="card-body py-2 px-1">' +
            '<div class="fw-bold" style="font-size:1.35rem;line-height:1.05;color:' + color + ';">' + big + '</div>' +
            '<div class="fw-semibold mt-1" style="font-size:0.7rem;">' + label + '</div>' +
            '<div class="text-muted" style="font-size:0.62rem;">' + sub + '</div>' +
            '</div></div></div>';
    }
    return card('#0D6EFD', a.correos, 'Correos', 'enviados en total') +
           card('#FD7E14', a.envioMs ? formatMs(a.envioMs) : '—', 'Construcción → envío',
                a.correos > 0 ? formatMs(a.msPorCorreo) + ' / correo' : 'tiempo total') +
           card('#6F42C1', a.visionMs ? formatMs(a.visionMs) : '—', 'Vision',
                a.atms > 0 ? formatMs(a.msPorTk) + ' / ticket' : 'tiempo total');
}

function renderPeriodoDia() {
    var sel = document.getElementById('met-dia-select');
    var cont = document.getElementById('met-dia-cards');
    if (!sel || !cont) return;
    var lista = metDiasMap[sel.value] || [];
    var a = aggregar(lista);
    document.getElementById('met-dia-n').textContent = a.n;
    cont.innerHTML = periodoCardsHtml(a);
}

function renderPeriodoMes() {
    var sel = document.getElementById('met-mes-select');
    var cont = document.getElementById('met-mes-cards');
    if (!sel || !cont) return;
    var lista = metMesesMap[sel.value] || [];
    var a = aggregar(lista);
    document.getElementById('met-mes-n').textContent = a.n;
    cont.innerHTML = periodoCardsHtml(a);
}

var MESES_ES = ['','Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];

// Llena los selectores de día y mes preservando la selección actual si sigue existiendo
function poblarSelectoresPeriodo(sesiones) {
    metDiasMap = {};
    metMesesMap = {};
    sesiones.forEach(function(s) {
        var f = s.fecha;
        if (!f) return;
        (metDiasMap[f] = metDiasMap[f] || []).push(s);
        var mes = f.slice(0, 7);
        (metMesesMap[mes] = metMesesMap[mes] || []).push(s);
    });

    var selD = document.getElementById('met-dia-select');
    if (selD) {
        var prevD = selD.value;
        var dias = Object.keys(metDiasMap).sort().reverse();
        selD.innerHTML = dias.map(function(d) {
            return '<option value="' + d + '">' + d + ' (' + metDiasMap[d].length + ')</option>';
        }).join('');
        selD.value = metDiasMap[prevD] ? prevD : (dias[0] || '');
        renderPeriodoDia();
    }

    var selM = document.getElementById('met-mes-select');
    if (selM) {
        var prevM = selM.value;
        var meses = Object.keys(metMesesMap).sort().reverse();
        selM.innerHTML = meses.map(function(m) {
            var p = m.split('-');
            return '<option value="' + m + '">' + MESES_ES[parseInt(p[1],10)] + ' ' + p[0] + ' (' + metMesesMap[m].length + ')</option>';
        }).join('');
        selM.value = metMesesMap[prevM] ? prevM : (meses[0] || '');
        renderPeriodoMes();
    }
}

function eliminarSesion(idx) {
    showConfirm('¿Seguro que querés borrar esta sesión? No se puede deshacer.', async function() {
        try {
            var res = await fetch('/api/metricas/' + idx, { method: 'DELETE' });
            var data = await res.json();
            if (data.status === 'success') {
        ...
        } catch (e) { showToast('Error al eliminar la sesión', 'error'); }
    }, 'Eliminar sesión');
}

function renderMetricas(sesiones) {
    var hoy = new Date().toISOString().split('T')[0];
    var nSesiones = sesiones.length;

    // ── Totales acumulados ────────────────────────────────────────────────
    document.getElementById('met-card-total').textContent = nSesiones;

    var elN = document.getElementById('met-n-sesiones');
    if (elN) elN.textContent = nSesiones;

    // Acumulado por día y por mes (selectores + cards)
    poblarSelectoresPeriodo(sesiones);

    // Tiempo total de trabajo (suma de todas las sesiones)
    var totalTiempoMs = sesiones.reduce(function(s, x) {
        return s + (x.duraciones && x.duraciones.total_ms ? x.duraciones.total_ms : 0);
    }, 0);
    var elTT = document.getElementById('met-card-tiempo-total');
    if (elTT) elTT.textContent = totalTiempoMs > 0 ? formatMs(totalTiempoMs) : '—';

    // Total correos enviados acumulados
    ...
    // Tiempo total en envíos (suma de correos_ms)
    var totalTiempoEnvioMs = sesiones.reduce(function(s, x) {
        return s + (x.duraciones && x.duraciones.correos_ms ? x.duraciones.correos_ms : 0);
    }, 0);
    var elTE = document.getElementById('met-card-tiempo-envio-total');
    if (elTE) elTE.textContent = totalTiempoEnvioMs > 0 ? formatMs(totalTiempoEnvioMs) : '—';

    // Tiempo total en Vision (suma de vision_ms)
    var totalTiempoVisionMs = sesiones.reduce(function(s, x) {
        return s + (x.duraciones && x.duraciones.vision_ms ? x.duraciones.vision_ms : 0);
    }, 0);
    var elTV = document.getElementById('met-card-tiempo-vision-total');
    if (elTV) elTV.textContent = totalTiempoVisionMs > 0 ? formatMs(totalTiempoVisionMs) : '—';
    var avgVisionTkGlobal = avgPerUnit('vision_por_tk_ms');
    var elVAvg = document.getElementById('met-card-vision-avg-tk');
    if (elVAvg) elVAvg.textContent = avgVisionTkGlobal ? formatMs(avgVisionTkGlobal) + ' / TK' : '—';

    // Total de tickets a los que corresponde el tiempo en Vision
    var totalVisionTks = sesiones.reduce(function(s, x) {
        return s + (x.duraciones && x.duraciones.vision_tks_count ? x.duraciones.vision_tks_count : 0);
    }, 0);
    var elVTotal = document.getElementById('met-card-vision-tk-total');
    if (elVTotal) elVTotal.textContent = totalVisionTks;

    // ── Promedios de sesión ───────────────────────────────────────────────
    var conTotal = sesiones.filter(function(s){ return s.duraciones && s.duraciones.total_ms; });
    var elProm = document.getElementById('met-card-promedio');
    var elMej  = document.getElementById('met-card-mejor');
    if (conTotal.length > 0) {
        ...
        if (elProm) elProm.textContent = formatMs(avgTotal);
        if (elMej)  elMej.textContent  = formatMs(bestTotal);
    } else {
        if (elProm) elProm.textContent = '—';
        if (elMej)  elMej.textContent  = '—';
    }
    ...
    // ── Promedios por fase ────────────────────────────────────────────────
    var conProcesoEnvio = sesiones.filter(function(s) { return s.duraciones && s.duraciones.correos_ms; });
    var avgProcesoEnvio = conProcesoEnvio.length > 0
        ? conProcesoEnvio.reduce(function(sum, s) { return sum + s.duraciones.correos_ms; }, 0) / conProcesoEnvio.length
        : 0;
    var elPE = document.getElementById('met-card-proceso-envio');
    if (elPE) elPE.textContent = avgProcesoEnvio ? formatMs(avgProcesoEnvio) : '—';
    ...
    if (canvas) {
        ...
        metricasChart = new Chart(canvas.getContext('2d'), {
            type: 'bar',
            data: {
                labels: ['Construir correos / ATM', 'Gestión Outlook / ATM', 'Vision / ticket'],
                datasets: [{
    ...
    sesiones.map(function(s, i) { return { s: s, idx: i }; })
            .slice(-20).reverse().forEach(function(item) {
        var s = item.s;
        var d = s.duraciones || {};
        var correoCount = s.correos_enviados || (s.correos_sucursal || 0) + (s.correos_terceros || 0);
        var correoDesglose = correoCount > 0 && (s.correos_sucursal || s.correos_terceros)
            ? '<br><span class="text-muted" style="font-size:0.65rem;">Suc:' + (s.correos_sucursal||0) + ' Cust:' + (s.correos_terceros||0) + '</span>'
            : '';
        var tr = document.createElement('tr');
        tr.innerHTML =
            ...
            '<td class="text-center" style="color:#0D6EFD;font-weight:600;">' + correoCount + correoDesglose + '</td>' +
            '<td class="small" style="color:#FD7E14;">' + (d.correos_ms ? formatMs(d.correos_ms) : '—') + '</td>' +
            '<td class="small" style="color:#FD7E14;">' + (d.scripts_por_atm_ms ? formatMs(d.scripts_por_atm_ms) + '<span class="text-muted">/ATM</span>' : '—') + '</td>' +
            '<td class="small" style="color:#6F42C1;">' + (d.vision_por_tk_ms ? formatMs(d.vision_por_tk_ms) + '<span class="text-muted">/TK</span>' : '—') + '</td>' +
            '<td class="small fw-bold">' + (d.total_por_atm_ms ? formatMs(d.total_por_atm_ms) + '<span class="text-muted">/ATM</span>' : (d.total_ms ? formatMs(d.total_ms) : '—')) + '</td>' +
            '<td class="text-center"><button class="btn btn-sm btn-link text-danger p-0" title="Eliminar sesión" onclick="eliminarSesion(' + item.idx + ')"><i class="bi bi-trash"></i></button></td>';
        tbody.appendChild(tr);
    });
}
